...
def least_cost_path(G, start, dest, cost, V):
    todo = {start: 0}
    parent = {}
    visited = set()
...
def cost(edge, v):
    cost = (abs(v[edge[0]][0]-v[edge[1]][0]) + abs(v[edge[0]][1]-v[edge[1]][1]))**0.5
    return cost

def pathcost(path, v):
    pathcost = 0
    for i in range(len(path)-1):
        pathcost += cost((path[i], path[i+1]), v)
    return pathcost
